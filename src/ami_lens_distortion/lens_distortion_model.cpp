...
AMI_DLL_CPP point2d<double> lens_distortion_model::inverse_evaluation(point2d<double> &p)
{
  ...
  double *a = (double*)malloc(sizeof(double)*(Na+1));
  a[0] = d[0];
  ...
  char name[300] /**INPUT FILE NAME */)
{
  ...
  d.clear();
	...
	int val = fscanf(f,"%s\n",name_paso);
  if(val == EOF)
    printf("Read error: distortion_center\n");

  val = fscanf(f,"%lf %lf \n",&(c.x),&(c.y));
  if(val == EOF)
    printf("Error reading the distortion center value\n");

  val = fscanf(f,"%s\n",name_paso);
  if(val == EOF) 
    printf("Read error: first_distortion_model\n");

  val = fscanf(f,"%d\n",&N);
  if(val == EOF)
    printf("Error reading number of components\n");

  for(i=0; i<N; i++)
  {
    val = fscanf(f,"%lf\n",&a);
    if(val == EOF) 
      printf("Error reading lens distortion parameter %d\n",i);
    d.push_back(a);
  }
