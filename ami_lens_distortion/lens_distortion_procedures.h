																			...
																			const double &image_amplification_factor)
{
	...
	if(paso>max_distance_corner) max_distance_corner=paso;
	...
	if(image_amplification_factor==1.){ // ALL CORRECTED IMAGE IS VISIBLE
...
//      t.y=(scale*height-height)*0.5;
	...
	else if(image_amplification_factor==2.){ // SCALE IS FITTED TO KEEP WIDTH VISIBLE
...
//      t.y=(scale*height-height)*0.5;
	...
	else{ // SCALE IS FITTED TO KEEP WIDTH VISIBLE
	...
	for (nc=0; nc<3; nc++)
	{
		...
		for(i=0; i<height; i++) {
			for(j=0; j<width; j++) {
				...
				if(ind>=max_distance_corner) continue;
